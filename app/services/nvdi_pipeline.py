...
def run_model(ndvi_path, output_prefix):
    ...
    model_path = "src/models/final_model_1.pth"
...
async def processar_imagem_completa(data: MLProcessRequest):
    ...
    red_path = f"src/data/raw/{data.id}_BAND15.tif"
    nir_path = f"src/data/raw/{data.id}_BAND16.tif"
    ndvi_tif = f"src/data/processed/{data.id}_ndvi.tif"
    ndvi_preview = f"src/data/processed/{data.id}_ndvi_preview.png"
